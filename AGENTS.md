...
- A probe window counts as a measurement only when every model's billed request count equals what that window sent, and consecutive windows are held apart by more than the worst analytics lag observed. Both rules exist because a live P1 run checkpointed four windows off two agreeing analytics reads that had not accounted for their own traffic, and one would have been published as a finding. `scripts/compression_bench/README.md` has the counts and the recovery path; `test_probe_settle.py` holds the line.
