...
- Only the two probes need silence, and only inside their measurement windows: they read account-level Cloudflare analytics filtered to the same models and request source the dictation app itself uses, so dictating inside a window corrupts that window. The main grid records cost and duration per response and is unaffected. `scripts/compression_bench/quiet_window.py` owns the announcements and the quiet-time estimate, and `window_log.py` owns the per-window checkpoints that let the probes be run in short idle stretches rather than one sitting.
- Cloudflare bills more Workers AI inferences than the client sends: 0% excess at 10 requests, 4% at 50, 18% for 50 sent alongside 150 others across four models. A probe window counts as a measurement only when every model's billed count has reached what that window sent and the excess above it stays inside what the platform produces, and consecutive windows are held apart by more than the worst analytics lag observed. The excess is recorded per window and compared across speeds, because P1's ratio only means anything when the rate is the same at both. `scripts/compression_bench/README.md` has the counts, the thresholds and the recovery path; `test_probe_settle.py` holds the line.
